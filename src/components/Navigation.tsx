...
export default function Navigation({ currentPage, onNavigate, onCartClick, onCatalog }: NavigationProps) {
  ...
  return (
    <header className="sticky top-0 z-50 relative">
      {/* Outer padding shell - transitions on scroll */}
      <div
        style={{
      ...
      >
        ...
        >
          <button
            ...
            className="shrink-0 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-kb-tertiary"
          >
            <img
              src="/images/kitchenbots-logo.svg"
              alt="KitchenBots"
              className={`w-auto object-contain transition-all duration-300 ${isScrolled ? 'h-9 md:h-10' : 'h-11 md:h-12'}`}
            />
          </button>
      ...
      {searchOpen && (
        <div className="absolute right-6 top-[calc(100%+8px)] w-[min(560px,calc(100%-3rem))] rounded-2xl border border-[#E2E8F0] bg-white p-4 shadow-xl lg:right-[80px] z-50">
          <form
            role="search"
